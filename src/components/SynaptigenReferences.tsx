...
import { SCIENTIFIC_CITATIONS, ScientificCitation } from '../data/synaptigenData';
import { ExternalLink, ChevronDown, ChevronUp, BookOpen, X } from 'lucide-react';

export const SynaptigenReferences: React.FC = () => {
  const [showAll, setShowAll] = useState(false);
  const [selectedCitation, setSelectedCitation] = useState<ScientificCitation | null>(null);

  const displayedCitations = showAll
    ? SCIENTIFIC_CITATIONS
    : SCIENTIFIC_CITATIONS.slice(0, 8);

  return (
    <section id="references" className="py-16 sm:py-24 bg-white text-slate-900 border-t border-slate-200">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Media / Scientific Publication Logos (Matches Screenshot 5 Top) */}
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Peer-reviewed research and clinical citations supporting the ingredients in Synaptigen™
          </p>
        </div>

        {/* 2-Column Citations List (Matches Screenshot 5) */}
        ...
        <div className="mt-12 text-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border border-slate-300 hover:border-teal-600 text-xs font-bold text-slate-700 hover:text-teal-800 transition-colors cursor-pointer bg-white shadow-sm"
          >
            <span>{showAll ? 'Show Fewer References' : `View All ${SCIENTIFIC_CITATIONS.length} References`}</span>
      ...
      {selectedCitation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 relative shadow-2xl border border-slate-200">
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-teal-700 uppercase tracking-wide mb-2">
