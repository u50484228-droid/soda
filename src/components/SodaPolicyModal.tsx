...
import { X, Mail, Phone, Truck, RefreshCw } from 'lucide-react';

export type SodaPolicyType = 'contact' | 'terms' | 'disclaimer' | 'privacy' | 'shipping' | 'refund' | 'order-support';

interface SodaPolicyModalProps {
  type: SodaPolicyType | null;
  onClose: () => void;
}

export const SodaPolicyModal: React.FC<SodaPolicyModalProps> = ({ type, onClose }) => {
  if (!type) return null;
      ...
      <div className="bg-white text-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-8 relative shadow-2xl border border-slate-200 my-8 max-h-[85vh] overflow-y-auto">
        ...
        {type === 'refund' && (
          <div className="space-y-4 text-xs sm:text-sm text-slate-700">
            ...
            <p>
              SodaTide® provides an unconditional 180-day money-back guarantee. You have a full 6 months from the date of purchase to evaluate the product.
            </p>
            <p className="font-semibold text-slate-900 bg-purple-50 p-3.5 rounded-2xl border border-purple-200">
              Empty-Bottle Policy: Even if you have finished all bottles down to the very last capsule, you may return the containers for a 100% refund of the purchase price.
            </p>
            ...
            <ol className="list-decimal pl-5 space-y-1.5 text-slate-600">
              <li>Contact support at <strong className="text-purple-900">contact@getSodaTide.com</strong> or through BuyGoods.</li>
        ...
        {type === 'contact' && (
          <div className="space-y-4 text-xs sm:text-sm text-slate-700">
            ...
            <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-200">
              <p><strong>Customer Support Email:</strong> contact@getSodaTide.com</p>
              <p><strong>Phone Support:</strong> 1 (800) 492-7182 (Mon-Fri 9:00 AM – 5:00 PM EST)</p>
              <p><strong>Fulfillment Center:</strong> SodaTide Health Logistics, 1420 S 4800 W, Salt Lake City, UT 84104</p>
            </div>
          </div>
        )}
        ...
        {type === 'order-support' && (
          <div className="space-y-4 text-xs sm:text-sm text-slate-700">
            ...
            <p>
              BuyGoods is the official retailer of SodaTide. For inquiries regarding billing, transactions, or order lookup, you can access the BuyGoods customer portal directly at <strong>www.buygoods.com/orderlookup</strong> or email <strong>support@buygoods.com</strong>.
            </p>
          </div>
        )}
